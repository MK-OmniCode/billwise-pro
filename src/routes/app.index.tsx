...
function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [{ count: pc }, { count: cc }, { count: bc }, { data: bd }] = await Promise.all([
      ...
      const revenue = (bd ?? []).reduce((s, b) => s + Number(b.total), 0);
    ...
    <div className="p-8 max-w-7xl">
        ...
        {cards.map((c) => {
          ...
          return (
            <Link key={c.label} to={c.href}>
              <Card className="hover:shadow-elegant transition-shadow cursor-pointer border-border/60">
      ...
      <Card className="border-border/60">
        <CardContent className="p-6">
          <h2 className="font-display text-xl font-bold mb-4">Recent Bills</h2>
          {recentBills.length === 0 ? (
            <div className="text-sm text-muted-foreground text-center py-8">
              No bills yet. <Link to="/app/bills" className="text-primary font-medium hover:underline">Create your first bill →</Link>
            </div>
          ) : (
            <div className="divide-y divide-border">
              {recentBills.map((b) => (
                <Link key={b.id} to="/app/bills/$id" params={{ id: b.id }} className="flex items-center justify-between py-3 hover:bg-muted/50 px-2 rounded-md -mx-2">
                  <div>
                    <div className="font-medium">{b.bill_no}</div>
                    <div className="text-xs text-muted-foreground">{b.party_snapshot?.name || "—"} • {b.bill_date}</div>
                  </div>
                  <div className="font-bold text-primary">{fmtINR(Number(b.total))}</div>
                </Link>
              ))}
            </div>
          )}
